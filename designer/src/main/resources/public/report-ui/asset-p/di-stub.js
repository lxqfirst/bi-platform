...
(function(NS) {
    ...
    var DEFAULT_URL = '/reportTemplate/complex/generateReport.action';
     ...
     * @event
     */
    function reloadReport(paramObj) {
        // FIXME:实现不是很好
        // 如果原来data不存在，直接添加reportImageId
        if (this._oLoadOpt.data === undefined) {
            this._oLoadOpt.data = 'reportImageId=' + paramObj.reportImageId;
        } 
        // 如果data存在
        else {
            var left;
            var right;
            // 用reportImageId=分隔
            var datas = this._oLoadOpt.data.split('reportImageId=');
            
            // 如果长度大于1说明原来的报表镜像参数存在
            if (datas.length > 1) {
                
                left = datas[0];
                right = datas[1];
                var rights = right.split('&');
                rights[0] = paramObj.reportImageId;
                right = rights.join('&');
                this._oLoadOpt.data = left + 'reportImageId=' + right;
                
            } else {
                this.oLoadOpt.data += '&reportImageId=' + paramObj.reportImageId;
            }
        }
      
        iframeLoad.call(this);
    }
    /**
     * iframe方式的重置大小
     *
