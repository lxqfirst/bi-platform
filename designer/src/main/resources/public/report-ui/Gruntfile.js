...
module.exports = function(grunt) {
    ...
    function start() {
        ...
        grunt.initConfig(
            {
                ...
                prop: (prop = {
                    dirBinBase: "asset-d",
                    dirBinCom: "asset-d/-com-",
                    dirBinConsoleBase: "asset",
                    dirBinStubBase: "asset-p",
                    dirSrc: "src",
                    dirSrcCore: "src/core",
                    dirSrcCSS: "src/css",
                    dirSrcImg: "src/css/img",
                    dirDIModel: "src/biz-shared/di.shared.model",
                    dirDIView: "src/biz-shared/di.shared.ui",
                    dirVUIAdapter: "src/biz-shared/di.shared.adapter",
                    dirInnerVUI: "src/biz-shared/di.shared.vui",
                    dirECUICore: "src/core/ecui/src-core",
                    dirECUIExt: "src/core/ecui/src-ext",
                    dirXUI: "src/core/xui-ui",
                    sBinComName: "-com-",
                    sVTPLDelimeter: "-o_o-",
                    sVTPLAll: "FALSE",
                    sCodeFlagBegin: "//==[DI=BEGIN]==[NIGEB=ID]==",
                    sCodeFlagEnd: "//==[DI=END]==[DNE=ID]==",
                    srcEcharts: "src/core/echarts/echarts-plain-original.js",
                    srcJquery: "src/core/jquery/jquery-1.7.1.js",
                    srcRepoDict: "src/biz-shared/di.config/repo-dict.js",
                    getDirBinProdBiz: function (bizKey) {
                        return [prop.dirBinBase, bizKey || prop.bizKey].join('/');
                    },
                    getDirBinProdCSS: function (phase, bizKey) {
                        return [prop.dirBinBase, bizKey || prop.bizKey, phase, 'css'].join('/');
                    },
                    getDirBinConsoleCSS: function () {
                        return [prop.dirBinConsoleBase, 'css'].join('/');
                    },
                    getBinProdJS: function (phase, suffix, bizKey) {
                        suffix === false && (suffix = '-' + prop.srcMode);
                        return [prop.dirBinBase, bizKey || prop.bizKey, phase, '-di-product' + (suffix || '') + '.js'].join('/');
                    },
                    getBinProdAllJS: function (phase, bizKey) {
                        return [prop.dirBinBase, bizKey || prop.bizKey, phase, '-di-product*.js'].join('/');
                    },
                    getBinConsoleJS: function (suffix) {
                        suffix === false && (suffix = '-' + prop.srcMode);
                        return [prop.dirBinConsoleBase, '-di-console' + (suffix || '') + '.js'].join('/');
                    },
                    getBinStubJS: function (suffix) {
                        suffix === false && (suffix = '-' + prop.srcMode);
                        return [prop.dirBinStubBase, 'di-stub' + (suffix || '') + '.js'].join('/');
                    },
                    getBinConsoleAllJS: function () {
                        return [prop.dirBinConsoleBase, '-di-console*.js'].join('/');
                    },
                    getBinProdCSS: function (phase, suffix, bizKey) {
                        // 表示依照srcMode
                        suffix === false && (suffix = '-' + prop.srcMode);
                        return [prop.dirBinBase, bizKey || prop.bizKey, phase, 'css', '-di-product' + (suffix || '') + '.css'].join('/');
                    },
                    getBinConsoleCSS: function (suffix) {
                        suffix === false && (suffix = '-' + prop.srcMode);
                        return [prop.dirBinConsoleBase, 'css', '-di-console' + (suffix || '') + '.css'].join('/');
                    },
                    prodAllImg: [
                        // TODO
                        // 后面精确一下
                    ...
                    consoleAllImg: [
                        ...
                        // 后面精确一下
                        "<%= prop.dirSrc %>/css/img/*",
                        "!<%= prop.dirSrc %>/css/img/*source*",
                        "!<%= prop.dirSrc %>/css/img/*.psd"
                    ],
                    getDirBinProdImg: function (phase, bizKey) {
                        return [prop.dirBinBase, bizKey || prop.bizKey, phase, 'css', 'img'].join('/');
                    },
                    getDirBinConsoleImg: function () {
                        return [prop.dirBinConsoleBase, 'css', 'img'].join('/');
                    },
                }),
                ...
                clean: {
                    toPre: {
                        src: ['<%= prop.getBinProdAllJS("pre") %>', '<%= prop.getDirBinProdCSS("pre") %>/*.css', '<%= prop.getDirBinProdCSS("pre") %>/img/*']
                    },
                    toRelease: {
                        src: ['<%= prop.getBinProdAllJS("release") %>', '<%= prop.getDirBinProdCSS("release") %>/*.css', '<%= prop.getDirBinProdCSS("release") %>/img/*']
                    },
                    rebuildBizDev: {
                        src: ['<%= prop.getBinProdAllJS("dev") %>', '<%= prop.getDirBinProdCSS("dev") %>/*.css', '<%= prop.getDirBinProdCSS("dev") %>/img/*']
                    },
                    toConsole: {
                        src: ['<%= prop.getBinConsoleAllJS() %>', '<%= prop.getDirBinConsoleCSS() %>/*.css', '<%= prop.getDirBinConsoleCSS() %>/img/*']
                    },
                    toStub: {
                ...
                concat: {
                    createBiz: {
                        files: [
                            { src: '<%= gen.srcAllJS %>', dest: '<%= prop.getBinProdJS("dev", "-single") %>' },
                            { src: '<%= gen.srcAllCSS %>', dest: '<%= prop.getBinProdCSS("dev", "-single") %>' }
                        ]
                    },
                    toPre: {
                        files: [
                            { src: '<%= gen.srcAllJS %>', dest: '<%= prop.getBinProdJS("pre", "-single") %>' },
                            { src: '<%= gen.srcAllCSS %>', dest: '<%= prop.getBinProdCSS("pre", "-single") %>' }
                        ]
                    },
                    toConsole: {
                        files: [
                            { src: '<%= gen.srcAllJS %>', dest: '<%= prop.getBinConsoleJS("-single") %>' },
                            { src: '<%= gen.srcAllCSS %>', dest: '<%= prop.getBinConsoleCSS("-single") %>' }
                        ]
                    },
                    toStub: {
                        files: [
                ...
                jsDebugGen: {
                    createBiz: {
                        files: [
                            { src: '<%= gen.srcAllJS %>', dest: '<%= prop.getBinProdJS("dev", "-debug") %>' }
                        ]
                    },
                    toPre: {
                        files: [
                            { src: '<%= gen.srcAllJS %>', dest: '<%= prop.getBinProdJS("pre", "-debug") %>' }
                        ]
                    },
                    toConsole: {
                        files: [
                            { src: '<%= gen.srcAllJS %>', dest: '<%= prop.getBinConsoleJS("-debug") %>' }
                        ]
                    },
                    toStub: {
                        files: [
                ...
                cssDebugGen: {
                    createBiz: {
                        files: [
                            { src: '<%= gen.srcAllCSS %>', dest: '<%= prop.getBinProdCSS("dev", "-debug") %>' }
                        ]
                    },
                    toPre: {
                        files: [
                            { src: '<%= gen.srcAllCSS %>', dest: '<%= prop.getBinProdCSS("pre", "-debug") %>' }
                        ]
                    },
                    toConsole: {
                        files: [
                            { src: '<%= gen.srcAllCSS %>', dest: '<%= prop.getBinConsoleCSS("-debug") %>' }
                        ]
                    }
                },
                uglify: {
                    createBiz: {
                        files: [
                            { src: '<%= prop.getBinProdJS("dev", "-single") %>', dest: '<%= prop.getBinProdJS("dev", "-min") %>' }
                        ]
                    },
                    toPre: {
                        files: [
                            { src: '<%= prop.getBinProdJS("pre", "-single") %>', dest: '<%= prop.getBinProdJS("pre", "-min") %>' }
                        ]
                    },
                    toConsole: {
                        files: [
                            { src: '<%= prop.getBinConsoleJS("-single") %>', dest: '<%= prop.getBinConsoleJS("-min") %>' }
                        ]
                    },
                    toStub: {
                        files: [
                ...
                cssmin: {
                    createBiz: {
                        files: [
                            { src: '<%= prop.getBinProdCSS("dev", "-single") %>', dest: '<%= prop.getBinProdCSS("dev", "-min") %>' }
                        ]
                    },
                    toPre: {
                        files: [
                            { src: '<%= prop.getBinProdCSS("pre", "-single") %>', dest: '<%= prop.getBinProdCSS("pre", "-min") %>' }
                        ]
                    },
                    toConsole: {
                        files: [
                            { src: '<%= prop.getBinConsoleCSS("-single") %>', dest: '<%= prop.getBinConsoleCSS("-min") %>' }
                        ]
                    }
                },
                copy: {
                    createBiz: {
                        files: [
                            { src: '<%= prop.getBinProdJS("dev", false) %>', dest: '<%= prop.getBinProdJS("dev") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdCSS("dev", false) %>', dest: '<%= prop.getBinProdCSS("dev") %>', filter: 'isFile', notnull: true },
                            {
                                // TODO
                                // 后面精确一下
                                expand: true,
                                src: '<%= prop.prodAllImg %>',
                                dest: '<%= prop.getDirBinProdImg("dev") %>/',
                                filter: 'isFile',
                                notnull: true,
                                flatten: true
                            }
                    ...
                    toPre: {
                        files: [
                            { src: '<%= prop.getBinProdJS("pre", false) %>', dest: '<%= prop.getBinProdJS("pre") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdCSS("pre", false) %>', dest: '<%= prop.getBinProdCSS("pre") %>', filter: 'isFile', notnull: true },
                            { expand: true, src: '<%= prop.getDirBinProdImg("dev") %>/*', dest: '<%= prop.getDirBinProdImg("pre") %>/', filter: 'isFile', notnull: true, flatten: true },
                            { expand: true, src: '<%= gen.vtplList %>', dest: '<%= prop.getDirBinProdBiz() %>/pre/', filter: 'isFile', notnull: true, flatten: true }
                        ]
                    },
                    rebuildToPre: {
                        files: [
                    ...
                    toRelease: {
                        files: [
                            { src: '<%= prop.getBinProdJS("pre") %>', dest: '<%= prop.getBinProdJS("release") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdJS("pre", "-single") %>', dest: '<%= prop.getBinProdJS("release", "-single") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdJS("pre", "-debug") %>', dest: '<%= prop.getBinProdJS("release", "-debug") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdJS("pre", "-min") %>', dest: '<%= prop.getBinProdJS("release", "-min") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdCSS("pre") %>', dest: '<%= prop.getBinProdCSS("release") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdCSS("pre", "-single") %>', dest: '<%= prop.getBinProdCSS("release", "-single") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdCSS("pre", "-debug") %>', dest: '<%= prop.getBinProdCSS("release", "-debug") %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinProdCSS("pre", "-min") %>', dest: '<%= prop.getBinProdCSS("release", "-min") %>', filter: 'isFile', notnull: true },
                            { expand: true, src: '<%= prop.getDirBinProdImg("pre") %>/*', dest: '<%= prop.getDirBinProdImg("release") %>/', filter: 'isFile', notnull: true, flatten: true },
                            { expand: true, src: '<%= gen.vtplList %>', dest: '<%= prop.getDirBinProdBiz() %>/release/', filter: 'isFile', notnull: true, flatten: true }
                        ]
                    },
                    rebuildToRelease: {
                        files: [
                    ...
                    toConsole: {
                        files: [
                            { src: '<%= prop.getBinConsoleJS(false) %>', dest: '<%= prop.getBinConsoleJS() %>', filter: 'isFile', notnull: true },
                            { src: '<%= prop.getBinConsoleCSS(false) %>', dest: '<%= prop.getBinConsoleCSS() %>', filter: 'isFile', notnull: true },
                            {
                                expand: true,
                                src: '<%= prop.consoleAllImg %>',
                                dest: '<%= prop.getDirBinConsoleImg() %>/',
                                filter: 'isFile',
                                notnull: true,
                                flatten: true
                            }
        ...
        grunt.registerTask('rebuildBizDev', taskRebuildBizDev);
        grunt.registerTask('rebuildBizAllPhase', taskRebuildBizAllPhase);
        grunt.registerTask('rebuildToPre', taskRebuildToPre);
        grunt.registerTask('rebuildToRelease', taskRebuildToRelease);
        grunt.registerTask('toPre', taskToPre);
        grunt.registerTask('toRelease', taskToRelease);
        grunt.registerTask('toConsole', taskToConsole);
     ...
     * 入口任务：重新构建某产品线的dev
     */
    function taskRebuildBizDev(bizKey, skin, srcMode, mock) {
        prepareConf(
            // 输入验证
            checkBaseInput({
                bizKey: bizKey,
    ...
    /**
     * 入口任务：从dev构建到pre中
     * grunt toPre:insight:m
     */
    function taskToPre(bizKey, skin, vtplName, srcMode, mock) {
        prepareConf(
            // 输入验证
            checkBaseInput({
                bizKey: bizKey,
        ...
        // 先检查
        requireProp('bizKey', 'skin', 'vtplName', 'srcMode', 'rangeMode');
        checkBizExists(prop.bizKey, true);
        checkSkinExists(prop.skin);
        ...
        runTask('analyzeSrcJSBiz');
        runTask('analyzeSrcCSSBiz');
        runTask('analyzeVTPLBiz');

        // 然后执行操作
        runTask('clean', 'toPre');
        runTask('concat', 'toPre');
        runTask('uglify', 'toPre');
        runTask('cssmin', 'toPre');
        runTask('jsDebugGen', 'toPre');
        runTask('cssDebugGen', 'toPre');
        runTask('copy', 'toPre');
    }
     ...
     * grunt toPre:insight:m
     */
    function taskRebuildToPre(bizKey, skin, vtplName, srcMode, mock) {
        prepareConf(
            // 输入验证
            checkBaseInput({
                bizKey: bizKey,
        ...
        runTask('jsDebugGen', 'toPre');
        runTask('cssDebugGen', 'toPre');
        runTask('copy', 'rebuildToPre');
    }
    
    /**
     * 入口任务：从pre构建到release中
     */
    function taskToRelease(bizKey, vtplName, srcMode, mock) {
        prepareConf(
            // 输入验证
            checkBaseInput({
                bizKey: bizKey,
        ...
        // 先检查
        requireProp('bizKey', 'vtplName', 'srcMode', 'rangeMode');
        checkBizExists(prop.bizKey, true);
        ...
        // 然后执行操作
        runTask('clean', 'toRelease');
        runTask('copy', 'toRelease');
    }
    
    /**
     * 入口任务：从pre构建到release中
     */
    function taskRebuildToRelease(bizKey, vtplName, srcMode, mock) {
        prepareConf(
            // 输入验证
            checkBaseInput({
                bizKey: bizKey,
        ...
        // 然后执行操作
        runTask('clean', 'toRelease');
        runTask('copy', 'rebuildToRelease');
    }

    /**
     * 入口任务：di console的构建
     */
    function taskToConsole(srcMode, mock) {
        prepareConf(
            // 输入验证
            checkBaseInput({
                srcMode: srcMode || 'min',
                mock: mock || false,
                rangeMode: 'consoleAll'
            })
        );
        ...
        // 然后分析
        runTask('analyzeSrcJSConsole');
        runTask('analyzeSrcCSSConsole');

        // 然后执行操作
        runTask('clean', 'toConsole');
        runTask('concat', 'toConsole');
        runTask('uglify', 'toConsole');
        runTask('cssmin', 'toConsole');
        runTask('jsDebugGen', 'toConsole');
        runTask('cssDebugGen', 'toConsole');
        runTask('copy', 'toConsole');
    }
